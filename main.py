#!pip install  nltk #install nltk caso não tenha instalado
import nltk
...
# Banco de Dado inicial para teste
...
# Palavras que  podem  ser removidas para deixar o código mais fluido, sem afetar o  que  o código busca (nltk  já tem  seu  propio)
stopwords = ['a', 'agora', 'algum', 'alguma', 'aquele', 'aqueles', 'de', 'deu', 'do', 'e', 'estou', 'esta', 'esta',
             'ir', 'meu', 'muito', 'mesmo', 'no', 'nossa', 'o', 'outro', 'para', 'que', 'sem', 'talvez', 'tem', 'tendo',
             'tenha', 'teve', 'tive', 'todo', 'um', 'uma', 'umas', 'uns', 'vou']  

def removestopwords(texto): # função que iremos  chamar para remover as palavras desnecessarias
  frases = []  # Lista para guardar as frases e  depois retornar
  for(palavras, emocao) in texto:  # vai rodar todas as palavras, e suas  respectivas emoções no  texto escolhido
    semstop = [p for  p in palavras.split() if  p not in stopwords] # Se a palavra não  estiver no stopwords, ele guarda no semstop
    frases.append((semstop, emocao)) # faz  um append das palavras na lista  que criamos acima
  return frases # retorna todas as frases, e  suas emoções, sem as stopwords
print(removestopwords(base))

stopwordsnltk =  nltk.corpus.stopwords.words('portuguese') # stopwords do propio nltk
stopwordsnltk.append('vou')
stopwordsnltk.append('tão')
print(stopwordsnltk) # printar, para mostrar quais stopwords o propio nltk disponibiliza

def removestopwords(texto): # função que iremos  chamar para remover as palavras desnecessarias
  frases = []  # Lista para guardar as frases e  depois retornar
  for(palavras, emocao) in texto:  # vai rodar todas as palavras, e suas  respectivas emoções no  texto escolhido
    semstop = [p for  p in palavras.split() if  p not in stopwordsnltk] # Se a palavra não  estiver no stopwords, ele guarda no semstop
    frases.append((semstop, emocao)) # faz  um append das palavras na lista  que criamos acima
  return frases # retorna todas as frases, e  suas emoções, sem as stopwords
print(removestopwords(base))
...
def aplicastemmer(texto):
  ...
  for(palavras, emocao) in texto:
    comstemming = [str(stemmer.stem(p)) for p in palavras.split() if p not in stopwordsnltk] #vai pegar os radicais, ja fazendo as stopwords
...
# Listagem de todas as palavras base ↓
def buscapalavras(frases): # ajuda a fazer a tabela
  todaspalavras = []
  for (palavras, emocao) in frases:
    todaspalavras.extend(palavras) # sem a emocao
  return todaspalavras

palavras = buscapalavras(frasescomstemming)
print(palavras)

# Extração de palavras unicas ↓
...
# Extração de cada frase ↓
def extratorpalavras(documento):
  doc = set(documento)
  caracteristicas = {}
  for palavras in palavrasunicas:
    caracteristicas['%s' % palavras] = (palavras in doc) #verificar no meu documento quais palavras tem, e comparar com as minhas palavras unicas
  return caracteristicas

caracteristicasfrase = extratorpalavras(['admir', 'am', 'nov', 'dia'])
print(caracteristicasfrase)

# Extração de todas as frases/base de dados final ↓
basecompleta = nltk.classify.apply_features(extratorpalavras, frasescomstemming) #frasescomstemming já tem radical e emocao
print(basecompleta[0])

# Naive Bayes ↓
classificador = nltk.NaiveBayesClassifier.train(basecompleta) # constroi a tabela de probabilidade
print(classificador.labels()) # emoções
print(classificador.show_most_informative_features(1)) # probabilidades de cada radical


teste = 'estou apavorado com essa situação' # 'eu te amo' falha, pois o radical de amo deu amo, e não am que tem no banco
testestemming = []
stemmer = RSLPStemmer()

# ↓ Basicamente um stemming, so que sem passar o parametro de emoção
for (palavras) in teste.split():
  comstem = [p for p in palavras.split()]
  testestemming.append(str(stemmer.stem(comstem[0])))
print(testestemming)

novo = extratorpalavras(testestemming) # Colocamos ele na nosso extrator de palavras, para ver quais palavras estão no texto e quais não estão
print(novo)

print(classificador.classify(novo)) #retorna o label (emoção: alegria ou medo)
distribuicao = classificador.prob_classify(novo)
for classe in distribuicao.samples():
  print("%s: %f" % (classe, distribuicao.prob(classe))) # printa o percentual da label (emoção: alegria ou medo)
  
###########################################
# Codigo final com o Banco de Dados final #
###########################################
...
# Codigo final com o Banco de Dados completo
def aplicastemmer(texto):
  from nltk.stem import RSLPStemmer
  stemmer = RSLPStemmer()
  #stemmer = nltk.stem.RSLPStemmer # lingua portuguesa
...
freasescomstemmingteste = aplicastemmer(baseteste)
...
def buscapalavrasunicas(frequenciatreinamento):
...
basecompletatreinamento = nltk.classify.apply_features(extratorpalavras, frasescomstemmingtreinamento) #frasescomstemmingtreinamento já tem radical e emocao
basecompletateste = nltk.classify.apply_features(extratorpalavras, frasescomstemmingteste)
#print(basecompletatreinamento[0])

###

classificador = nltk.NaiveBayesClassifier.train(basecompletatreinamento) # constroi a tabela de probabilidade
#print(classificador.labels()) # emoções
#print(classificador.show_most_informative_features(5)) # probabilidades de cada radical

###

#print(nltk.classify.accuracy(classificador, basecompletatreinamento)) #  mesmas frases de treinamento e de teste
#print(nltk.classify.accuracy(classificador, basecompletateste)) #  diferentes frases de treinamento e de  teste

erros  = []
for  (frase,  classe) in basecompletateste: #  ↓  pegar classes erradas para suas frases
  #print(frase)
  #print(classe)
...
#esperado = 'alregria alegria alegria alegria medo medo surpresa surpresa'.split()
#previsto =  'alegria alegria medo  surpresa medo medo medo surpresa'.split()
matriz  =  ConfusionMatrix(esperado, previsto)
print(matriz)

#  1. cenário - 32%
#  2. número de classes (random  ou  classe)  - 16%
#  3. ZeroRules  - 21,05%

###

teste = 'eu sinto  amor por voce' # 'eu te amo' falha, pois o radical de amo deu amo, e não am que tem no banco
testestemming = []
stemmer = RSLPStemmer()
